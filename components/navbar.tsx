...
export default function Navbar() {
    return (
        <nav className="sticky top-8 flex rounded-full overflow-hidden bg-zinc-800 text-zinc-50 h-12 text-sm pl-0 group z-50">
            <div className="nav__content grid">
                <Link href="/" className="w-12 grid place-items-center text-zinc-50">
                ...
                <ul className="list-none flex h-full items-center p-[0_1rem] m-0">
                    {siteConfig.navItems.map((item, i) => (
                        <li key={item.label + i} className={`text-center h-full flex items-center justify-center [--i:${i}] w-[var(--item)] [opacity:var(--expanded)] [transition:opacity_var(--transition)_calc(0.75s+(var(--i)*0.05s))]`}>
                            <Link href={item.href} className="text-zinc-50 w-full h-full grid place-items-center whitespace-nowrap">
                                <span>{item.label}</span>
                            </Link>
                        </li>

                    ))}
                    <li className="text-center h-full flex items-center justify-center [--i:3] w-[var(--item)] [opacity:var(--expanded)] [transition:opacity_var(--transition)_calc(0.75s+(var(--i)*0.05s))]"><ThemeSwitch /></li>
                </ul>
                <a className="rounded-full flex gap-[0_0.5rem] items-center bg-zinc-800 text-zinc-50 whitespace-nowrap z-50 cursor-pointer relative p-[0_var(--gap)] before:content-none before:absolute before:right-full before:w-12 before:[aspect-ratio:1] before:[scale:calc(1-var(--expanded))1] before:[transition:scale_var(--transition)] before:[transform-origin:100%_50%]">
